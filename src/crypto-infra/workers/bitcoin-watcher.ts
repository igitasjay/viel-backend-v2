...
const BITCOIN_API_BASE = config.BITCOIN_API_URL || "https://blockstream.info/api";
    ...
    try {
      ...
      for (const wallet of btcWallets) {
        try {
          ...
          const response = await axios.get(`${BITCOIN_API_BASE}/address/${wallet.address}/txs`);
          ...
          const vout = latestTx.vout.find((v: any) => v.scriptpubkey_address === wallet.address);
          if (!vout) continue;

          const amountBTC = vout.value / 100000000; // Satoshis to BTC

          const coin = await Currency.findOne({ symbol: wallet.currency, network: 'BITCOIN' });
          ...
          logger.info(`✅ Bitcoin Deposit Processed! ${amountBTC} BTC for ${wallet.address}`);
          ...
          logger.error(`Error checking Bitcoin address ${wallet.address}:`, err.message);
        }
      }
    } catch (error: any) {
