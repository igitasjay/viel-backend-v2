...
export const getSellBrands = asyncHandler(
...
export const sellGiftCard = asyncHandler(
  async (req: Request, res: Response) => {
    const { brandId, country, range, type, amount, quantity, comment } =
      req.body;
    ...
    if (!typeData.denominations.includes(Number(amount))) {
      throw new ApiError(
        400,
        `Invalid amount. Supported: ${typeData.denominations.join(', ')}`,
      );
    }
