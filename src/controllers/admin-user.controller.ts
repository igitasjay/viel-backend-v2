...
export const suspendUserAccount = asyncHandler(async (req: Request, res: Response) => {
  ...
  res.json({ success: true, message: 'Account suspended successfully', data: user });
});

// Activation (Bonus/Helper)
export const activateUserAccount = asyncHandler(async (req: Request, res: Response) => {
  const userId = req.query.userId as string;
  ...
  res.json({ success: true, message: 'Account activated successfully', data: user });
});
...
export const deleteUserAccount = asyncHandler(async (req: Request, res: Response) => {
  ...
  res.json({ success: true, message: 'Account marked as deleted successfully' });
});
...
export const getUserTransactionHistory = asyncHandler(async (req: Request, res: Response) => {
  const userId = req.query.userId as string;
  const page = parseInt(req.query.page as string) || 1;
  const limit = parseInt(req.query.limit as string) || 20;
  const skip = (page - 1) * limit;
